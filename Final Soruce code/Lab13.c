...
#define alarm_sw        PORTAbits.RA4
#define heater_sw       PORTAbits.RA5
#define Chip_Select     PORTCbits.RC0
#define heater_on_led   PORTDbits.RD7

#define RGB1_R          PORTDbits.RD0
#define RGB1_G          PORTDbits.RD1
#define RGB1_B          PORTDbits.RD2

#define RGB2_R          PORTDbits.RD3
#define RGB2_G          PORTDbits.RD4
#define RGB2_B          PORTDbits.RD5

// initialize variables for corresponding data
...
unsigned char alarm_mode, alarm_match_flag, alarm_clear_flag, alarm_on_flag, alarm_sw_saved;
...
// high priority interrupt for alarm reset, alarm alert signal, IR remote key
// detecting
void interrupt high_priority chkisr()
{
...
// INT0 interrupt for the detection of the Alarm Reset
void INT0_isr(void)
{
    INTCONbits.INT0IF = 0;
alarm_clear_flag = 1;
...
// INT1 interrupt for the Alarm Alert signal asserted by the RTC when the Alarm
// Mode is on
void INT1_isr(void)
{
...
// Read the content of the timer and save into a variable
// Determine to update the state of "nec_state" depending on the time measured
void INT2_isr(void)
{
    unsigned int timer;    
    INTCON3bits.INT2IF = 0;             // Clear external interrupt
 
    if (nec_state != 0)                 // Check if nec_state is not in state 0
    {      
    ...
    switch(nec_state)                   // Determine to update the state of "nec_state" depending on the time measured
    {      
        case 0 :                        // This is to handle first detection of Leading Pulse  
            ...
            cnt = 0;                    // Force bit count (bit_count) to 0
            nec_code = 0;               // Set nec_code = 0
            INTCON2bits.INTEDG2 = 1;    // Change Edge interrupt of INT2 to Low to High to wait for the end of the Leading Pulse
            nec_state = 1;              // Set nec_state to state 1
            return;
       
        case 1 :                                // This is to handle the end of the Leading Pulse and start of Space
            if (timer > 8500 && timer < 9500)   // Check if Timer1 value read is between 8500 usec and 9500 usec
            {
                INTCON2bits.INTEDG2 = 0;        // If yes, change Edge interrupt of INT2 to High to Low to handle end of Space
                nec_state = 2;                  // Set nec_state to state 2
            }
            else
                nec_state = 0;                  // If not, force nec_state = 0
            return;
       
        case 2 :                                // This is to handle the end of Space
            if (timer > 4000 && timer < 5000)   // Check if Timer1 value read is between 4000 usec and 5000 usec
            {
                INTCON2bits.INTEDG2 = 1;        // If yes, change Edge interrupt of INT2 to Low to High to handle the start of bit detection
        ...
        case 3 :                                // This is the start of the loop to detect the bit of the data sequence
            if (timer > 400 && timer < 700)     // Check if Timer1 value read is between 400 usec and 700 usec
            {
                INTCON2bits.INTEDG2 = 0;        // If yes, change Edge interrupt of INT2 to Low to High
        ...
        case 4 :                                // This is the start of the loop to detect the bit of the data sequence
            if (timer > 400 && timer < 1800)    // Check if Timer1 value read is between 400 usec and 1800 usec
            {
                nec_code = nec_code << 1;       // If yes, shift left nec_code by 1 bit
                         
                if (timer > 1000)               // Check if Timer1 is greater than 1000 usec
                    nec_code = nec_code + 1;    // If yes, add 1 to nec_code
               
                cnt++;                          // Increment bit_count by 1
               
                if (cnt > 31)                   // Check if bit_count > 31
                {
                    nec_ok = 1;                 // If yes, set nec_ok flag to 1  
                    INTCON3bits.INT2IE = 0;     // and set INT2IE = 0
                }
                else
                {
                    INTCON2bits.INTEDG2 = 1;    // Change Edge interrupt of INT2 to High to Low
                    nec_state = 3;              // Set nec_state to state 3
                }
            }
            else
...
// Reset decoding process, INT2 interrupts on falling edge, Disable Timer1, and
...
void main()
{
    init_UART();
    OSCCON = 0x70;                      // 8 MHz
    ADCON1 = 0x0B;                      // Configure pin AN0 - AN3 to digital
    nRBPU = 0;                          // Enable PORTB internal pull up resistor
    TRISA = 0x3F;                       // PORT A bit 0 to 5 as input
    TRISB = 0x07;                       // PORT B bit 0 to 2 as input
    TRISC = 0x00;                       // PORT C all bits as output
    TRISD = 0x00;                       // PORT D all bits as output
  TRISE = 0x00;                       // PORT E all bits as output
    CCP2CON = 0x0;                      // Capture/Compare/PWM off
...
INTCONbits.GIE = 1; // Enable global interrupt
    ...
    DS3231_Init(DS3231_ID);             // Initialize RTC
    set_heater_temp = 60;               // Set initial temp for heater
   
    Initialize_Main_Screen();
   
    TMR1L = 0;                          // Reset Timer1
    TMR1H = 0;                          //
    T1CON = 0x83;                       // 16-bit, external clock, enable Timer1
    PIR1bits.TMR1IF = 0;                // Clear interrupt flag
    PIE1bits.TMR1IE = 1;                // Enable Timer 1 interrupt    
    nec_ok = 0;                         // Clear IR remote flag
    nec_code = 0x0;                     // Clear IR remote code
    alarm_mode = 0;                     // Clear alarm mode flag
    heater_mode = 0;                    // Clear heater mode flag
    alarm_match_flag = 0;               // Clear alarm match flag
    LED_Intensity = 0;                  // Reset LED intensity
    SPI_out(LED_Intensity + 100);       // Output LED intensity
    PORTD = 0;                          // Clear PORT D
   
    while(1)
    {    
        DS3231_Read_Time(DS3231_ID);                // Read time from RTC
        if (second != old_second)                   // Check if second changes
        {
            alarm_mode = alarm_sw;                  // Change alarm mode according to alarm switch
            heater_mode = heater_sw;                // Change heater mode according to heater switch
               
            if (heater_mode == 1)                   // If heater mode is on
            {
                if (DS3231_tempF < set_heater_temp) // If read temp is higher than set temp
                    heater_on_led = ON;             // Turn on heater LED
                else
                    heater_on_led = OFF;            // Otherwise, turn off heater LED
            }
            else
                heater_on_led = OFF;                // Otherwise, turn off heater LED
           
            if (alarm_mode == 1)                    // If alarm mode is on
            {
            ...
            {
                INTCON3bits.INT1IE = 0;             // Otherwise, disable INT1
                INTCON3bits.INT1IF = 0;             // Clear INT1 flag
            }
                                   
            if (alarm_on_flag == 1)                 // If alarm on flag is set
            {
                // RGB LED at D1 changes with 8 different colors every second
                if ((RGB1_R == 1) && (RGB1_G == 1) && (RGB1_B == 1))
                {
                    RGB1_R = 0;
                    RGB1_G = 0;
                    RGB1_B = 0;  
            ...
            DS3231_tempC = DS3231_Read_Temp(DS3231_ID);     // Read temp              
            Do_Update_Main_Screen();                        // Update main screen
            printf("Time: %02x:%02x:%02x  Date: %02x:%02x:%02x  ", hour, minute,
                    second, month, day, year);
            printf("Temp: %2d C %2d F\r", DS3231_tempC, DS3231_tempF);
            old_second = second;                            // update reference second
        }
       
        if (alarm_clear_flag == 1)                          // If alarm clear flag is set
        {
            alarm_clear_flag = 0;                           // Clear alarm clear flag
            turn_off_sound();                               // Turn off buzzer
            PORTBbits.RB3 = 0;                              //
            RGB1_R = 0;                                     // Turn off RGB D1
            RGB1_G = 0;                                     //
            RGB1_B = 0;                                     //
            alarm_on_flag = 0;                              // Clear alarm on flag
        }
       
        if (alarm_match_flag == 1)                          // If alarm match flag is set
        {
            alarm_match_flag = 0;                           // Clear alarm match flag
            gen_2khz_sound();                               // Turn on buzzer
            alarm_on_flag = 1;                              // Set alarm on flag
           
            // Turn on alarm if alarm hasn't been turned on before
            if ((alarm_sw == 1) && (alarm_sw_saved == 0))  
            {
                DS3231_Turn_On_Alarm(DS3231_ID);
                alarm_sw_saved = alarm_sw;
            }
           
            // Turn off alarm if alarm has been turned on before
            if ((alarm_sw == 0) && (alarm_sw_saved == 1))
            {
        ...
        int STEP;
        float Volt;
        Select_ADC(0);                                      // Select analog input channel
        STEP = Get_Full_ADC();                              // Read data from analog input pin
        Volt = STEP * 4.0 / 10000.0;                        // Calculate input voltage
        printf("Volt = %f \r\n", Volt);
       
        // Turn on RGB D2 if voltage signal from light sensor is higher than 7V
        if (Volt > 7)
        {
            RGB2_R = 1;
            RGB2_G = 1;
        ...
        else                                                // Otherwise, turn off RGB D2
        {
            RGB2_R = 0;
            RGB2_G = 0;
            RGB2_B = 0;
...
// Update main screen data and text
void Do_Update_Main_Screen(void)
{
...
// Initialize time setup screen text
void Initialize_Setup_Time_Screen(void)
{
    fillScreen(ST7735_BLACK);                                       // Fills background of screen with color passed to it
...
// Update screen setup cursor location
...
// Change time according to IR remote signal
...
// Initialize alarm setup screen text
void Initialize_Setup_Alarm_Screen(void)
{
    fillScreen(ST7735_BLACK);                                   // Fills background of screen with color passed to it
    ...
    strcpy(txt, "AlarmSetup");                                  // Text displayed
...
// Update alarm setup screen data
void Update_Setup_Alarm_Screen(void)
{
...
// Change alarm time according to IR remote signal
void Do_Setup_Alarm_Time(void)
{
    char Key_Next_Flag, Key_Up_Flag, Key_Dn_Flag, Key_Abort_Flag, Key_Done_Flag;
    char Select_Field, Setup_Mode;    
...
// Change heater temp according to IR remote signal
void Do_Setup_Heater_Temp(void)
{
    char Key_Up_Flag, Key_Dn_Flag, Key_Abort_Flag, Key_Done_Flag;
    char Setup_Mode;    
    Setup_Mode = 1;
    DS3231_Read_Temp(DS3231_ID);
setup_heater_temp = set_heater_temp;
    ...
    while (Setup_Mode == 1)
    {
        if (nec_ok == 1)
        {
            ...
            {
                setup_heater_temp++;
                if (setup_heater_temp == 99) setup_heater_temp = 0;                  
                Update_Setup_Heater_Screen();
                Key_Up_Flag = 0;
            }
            ...
            if (Key_Dn_Flag == 1)
            {
                if (setup_heater_temp == 0) setup_heater_temp = 99;
                else --setup_heater_temp;              
                Update_Setup_Heater_Screen();
                Key_Dn_Flag = 0;                      
            }
...
// Initialize heater setup screen text
void Initialize_Setup_Heater_Screen(void)
{
    fillScreen(ST7735_BLACK);                                               // Fills background of screen with color passed to it
...
// Update heater setup screen date
void Update_Setup_Heater_Screen(void)
{
    setup_heater_S_text[0] = (setup_heater_temp/10)  + '0';
    setup_heater_S_text[1] = (setup_heater_temp%10)  + '0';
...
// Read value from selected analog channel
unsigned int Get_Full_ADC(void)
{
    int result;
...
// Select ADCON channel
void Select_ADC(char ADC_Channel)
{
    ADCON0 = 0x01 + ADC_Channel * 4;
}

// Calculate output value for LED intensity
void Do_LED_Out(char value)
{
    SPI_out(500/2/9*(value/10));
}
...
if(LED_Intensity > 0x5A) LED_Intensity = 0;
    Do_LED_Out(LED_Intensity);
}
...
{
    LED_Intensity = LED_Intensity - 10;
if(LED_Intensity < 0) LED_Intensity = 0x5A;
    Do_LED_Out(LED_Intensity);
}

// Output LED intensity
